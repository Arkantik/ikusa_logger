...
const error_message_mapping = {
  ...
  "Could not locate config file or config is invalid": "Could not locate config file or config is invalid",
...
const logger_callback: LoggerCallback = (data: string, status: "running" | "error" | "terminated") => {
  if (status === "running") {
